...
# Créer le Namespace
kubectl create ns $VOS_INITIALES -o=yaml --dry-run=client > tp3.yaml
echo "---" >> ./tp3.yaml
kubectl apply -f ./tp3.yaml
...
```bash
kubectl create deployment --replicas=3 whoami --image traefik/whoami:v1.11.0 --dry-run=client -o=yaml >> ./tp3.yaml
echo "---" >> ./tp3.yaml
kubectl apply -f ./tp3.yaml
```
...
- Exposer le service
```bash
kubectl expose deployment whoami --port=8080 --target-port=80 --name whoami --dry-run=client -o=yaml >> ./tp3.yaml
echo "---" >> ./tp3.yaml
kubectl apply -f ./tp3.yaml
```
...
```bash
kubectl get po,service -n ingress-nginx -o wide
...
```bash
NOM="$VOS_INITIALES"
kubectl create ingress whoami --class=nginx --rule="$NOM.onati.devops-experience.com/=whoami:8080" --dry-run=client -o=yaml >> ./tp3.yaml
echo "---" >> ./tp3.yaml
kubectl apply -f ./tp3.yaml
```
...
- Tester le service
```bash
kubectl run -it --rm --image=curlimages/curl:latest -- sh
